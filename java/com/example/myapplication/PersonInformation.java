...
public class PersonInformation extends AppCompatActivity {
    ...
    Button change_reset,change_con,change;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        change_reset = (Button) findViewById(R.id.change_reset);
        change_con = (Button) findViewById(R.id.change_con);
...
// 获取前四天的日期
        ...
        entries.add(new BarEntry(0, Login.currentUser.Time4));
        entries.add(new BarEntry(1, Login.currentUser.Time3));
        entries.add(new BarEntry(2, Login.currentUser.Time2));
        entries.add(new BarEntry(3, Login.currentUser.Time1));
        entries.add(new BarEntry(4, Login.currentUser.todayTime));
        ...
        imageButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("fromPersonInformation", true);
        ...
        change.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                changedName = Login.currentUser.name;
                changedBirthday = Login.currentUser.birthday;
                changedAge = Login.currentUser.age;
                changedGender = Login.currentUser.gender;
                changedSchool = Login.currentUser.school;
                changedClass = Login.currentUser.schoolClass;
    ...
    public void changeData() throws ParseException {
        if(!String.valueOf(change_name.getText()).isEmpty()) {
            ...
            Login.currentUser.name = changedName;
        }
        ...
        if(!String.valueOf(change_birthday.getText()).isEmpty()) {
            ...
            Login.currentUser.birthday = changedBirthday;
        }
        if(!String.valueOf(change_birthday.getText()).isEmpty()) {
            changedAge = Login.currentUser.getAge(changedBirthday);
            Login.currentUser.age = changedAge;
        }
        if(!String.valueOf(change_school.getText()).isEmpty()) {
            changedSchool = String.valueOf(change_school.getText());
            Login.currentUser.school = changedSchool;
        }
        if(!String.valueOf(change_class.getText()).isEmpty()) {
            changedClass = String.valueOf(change_class.getText());
            Login.currentUser.schoolClass = changedClass;
        }
        if(!String.valueOf(change_gender.getText()).isEmpty()) {
            ...
            Login.currentUser.gender = changedGender;
        }
        updateData();
        postData();
    ...
    private void postData() {
        Thread thread = new Thread(() -> {
            String g = "M";
            if(Login.currentUser.gender==0)
                g = "F";
            // 创建一个RequestBody对象，包含你要发送的数据
            RequestBody body = new FormBody.Builder()
                    .add("name", Login.currentUser.name)
                    .add("birthday", Login.currentUser.birthday)
                    .add("gender",g)
                    .add("school", Login.currentUser.school)
                    .add("class_name", Login.currentUser.schoolClass)
                    .build();
            // 创建一个Request对象
    ...
    public void updateData() throws ParseException {
        today_hour.setText(Integer.toString(Login.currentUser.todayTime/60));
        System.out.println(Login.currentUser.todayTime/60);
        today_min.setText(Integer.toString(Login.currentUser.todayTime- Login.currentUser.todayTime/60*60));
        total_hour.setText(Integer.toString(Login.currentUser.totalTime/60));
        total_min.setText(Integer.toString(Login.currentUser.totalTime- Login.currentUser.totalTime/60*60));
        name.setText(Login.currentUser.name);
        ...
        data.setText(sdf2.format(Objects.requireNonNull(sdf1.parse(Login.currentUser.birthday))));
        age.setText(Integer.toString(Login.currentUser.age));
        if(Login.currentUser.gender == 0)
            gender.setText("女");
        else
            gender.setText("男");
        school.setText(Login.currentUser.school);
        classroom.setText(Login.currentUser.schoolClass);
